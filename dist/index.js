...
const FloatingChatWidget = (props) => {
    ...
    const [sessionId] = React.useState(() => config.sessionId ||
        `fcw-${Math.random().toString(36).slice(2)}${Date.now()}`);
    const [isOpen, setIsOpen] = React.useState(false);
    const [messages, setMessages] = React.useState([]);
    const [inputValue, setInputValue] = React.useState("");
    const [isLoading, setIsLoading] = React.useState(false);
    const [isNagging, setIsNagging] = React.useState(false);
    const [showPrompts, setShowPrompts] = React.useState(true); // Hide after user sends first message
    ...
    const renderBubbleIcon = () => {
        if (typeof config.bubbleIcon === "string") {
            ...
            if (isImageUrl(config.bubbleIcon)) {
                return (React.createElement("img", { src: config.bubbleIcon, alt: "Chat", className: "fcw-bubble-icon-img" }));
            }
            // Otherwise treat as emoji or text
            return (React.createElement("span", { className: "flex items-center justify-center text-lg w-6 h-6 leading-none" }, config.bubbleIcon));
        }
    ...
    const renderTitleIcon = () => {
        ...
        if (isImageUrl(config.titleIcon)) {
            return (React.createElement("img", { src: config.titleIcon, alt: config.title, className: "fcw-title-icon-img" }));
        }
        // Otherwise treat as emoji or text
        return React.createElement("span", { className: "fcw-title-icon-emoji" }, config.titleIcon);
    };
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: `fcw-bubble-container ${positionClass}`, style: { zIndex: config.zIndex } },
            isNagging && config.nagMessage && (React.createElement("div", { className: `fcw-nag-tooltip ${config.position === "bottom-left" ? "fcw-nag-tooltip-left" : "fcw-nag-tooltip-right"}`, onClick: toggleWidget }, config.nagMessage)),
            React.createElement("button", { onClick: toggleWidget, className: `fcw-chat-bubble-btn ${isNagging ? "fcw-nagging" : ""}`, style: {
                    backgroundColor: config.cleanTheme
                        ? "transparent"
                        : config.themeColor,
                }, "aria-label": "Toggle chat" }, renderBubbleIcon())),
        React.createElement("div", { className: `fcw-chat-widget ${positionClass} ${isOpen ? "fcw-open" : "fcw-closed"}`, style: {
                zIndex: config.zIndex,
                width: config.width,
                height: config.height,
                fontFamily: config.fontFamily,
            } },
            React.createElement("div", { className: "fcw-chat-header", style: {
                    backgroundColor: config.cleanTheme ? "inherit" : config.themeColor,
                } },
                React.createElement("div", { className: "fcw-header-content" }, renderTitleIcon()),
                React.createElement("button", { onClick: toggleWidget, className: "fcw-close-btn", "aria-label": "Close chat" },
                    React.createElement("svg", { width: "16", height: "16", viewBox: "0 0 16 16", fill: "none", xmlns: "http://www.w3.org/2000/svg" },
                        React.createElement("path", { d: "M12.8536 4.85355C13.0488 4.65829 13.0488 4.34171 12.8536 4.14645C12.6583 3.95118 12.3417 3.95118 12.1464 4.14645L8 8.29289L3.85355 4.14645C3.65829 3.95118 3.34171 3.95118 3.14645 4.14645C2.95118 4.34171 2.95118 4.65829 3.14645 4.85355L7.29289 9L3.14645 13.1464C2.95118 13.3417 2.95118 13.6583 3.14645 13.8536C3.34171 14.0488 3.65829 14.0488 3.85355 13.8536L8 9.70711L12.1464 13.8536C12.3417 14.0488 12.6583 14.0488 12.8536 13.8536C13.0488 13.6583 13.0488 13.3417 12.8536 13.1464L8.70711 9L12.8536 4.85355Z", fill: "currentColor" })))),
            React.createElement("div", { className: "fcw-chat-messages" },
                messages.map((msg) => (React.createElement("div", { key: msg.id, className: `fcw-message ${msg.sender === "user" ? "fcw-message-user" : "fcw-message-bot"}` },
                    React.createElement("div", { className: `fcw-message-bubble ${msg.sender === "user"
                ...
                showPrompts && config.suggestedPrompts && config.suggestedPrompts.length > 0 && (React.createElement("div", { className: "fcw-suggested-prompts" }, config.suggestedPrompts.map((prompt, index) => (React.createElement("button", { key: index, className: "fcw-prompt-btn", onClick: () => handlePromptClick(prompt), type: "button" }, prompt))))),
                isLoading && (React.createElement("div", { className: "fcw-message fcw-message-bot" },
                    React.createElement("div", { className: "fcw-message-bubble fcw-message-bubble-bot" },
                        React.createElement("span", { className: "fcw-loading" }, "\u25CF\u25CF\u25CF")))),
                React.createElement("div", { ref: messagesEndRef })),
            React.createElement("form", { onSubmit: handleSubmit, className: "fcw-chat-input-form" },
                React.createElement("input", { ref: inputRef, type: "text", placeholder: config.placeholder, value: inputValue, onChange: (e) => setInputValue(e.target.value), className: "fcw-chat-input", style: { fontFamily: config.fontFamily } }),
                React.createElement("button", { type: "submit", disabled: isLoading || !inputValue.trim(), className: "fcw-chat-send-btn", "aria-label": "Send message" },
                    React.createElement(SvgSendIcon, { className: "fcw-send-icon" }))))));
};

exports.FloatingChatWidget = FloatingChatWidget;
exports.default = FloatingChatWidget;
